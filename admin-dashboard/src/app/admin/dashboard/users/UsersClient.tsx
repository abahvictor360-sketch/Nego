    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{users.length} merchant accounts</p>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="bg-white border border-gray-200 rounded-lg pl-9 pr-3 py-2 text-sm text-gray-900 placeholder-gray-500 outline-none focus:border-green-800 w-full sm:w-64"
          />
        </div>
      </div>
              ...
              {filtered.map(u => {
                ...
                return (
                  <tr key={u.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3">
                      {u.plan === 'pro' ? (
                        <span className="inline-flex items-center gap-1 text-xs font-semibold text-green-800 bg-green-900/10 px-2 py-1 rounded-md">
                          <Crown className="w-3 h-3" /> Pro
                        </span>
                      ) : (
                        <span className="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-1 rounded-md">Free</span>
                      )}
                    </td>
                    <td className="px-5 py-3">
                      {u.status === 'suspended' ? (
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          disabled={busy}
                          className="text-xs font-medium px-3 py-1.5 rounded-lg border border-gray-200 text-gray-700 hover:border-green-800 hover:text-green-700 transition-colors disabled:opacity-50"
                        >
