...
const STATUS_META: Record<string, { label: string; cls: string; Icon: typeof Circle }> = {
  ...
  pending: { label: 'Pending', cls: 'text-amber-400 bg-amber-500/10', Icon: Clock },
  closed: { label: 'Closed', cls: 'text-gray-400 bg-gray-700/40', Icon: CheckCircle2 },
};
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">{tickets.filter(t => t.status !== 'closed').length} open tickets</p>
      </div>

      <div className="grid lg:grid-cols-[320px_1fr] gap-4">
        {/* Ticket list */}
        <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden max-h-[70vh] overflow-y-auto">
            ...
            return (
              <button
                ...
                className={`w-full text-left px-4 py-3 border-b border-gray-800 transition-colors ${
                  active ? 'bg-amber-500/10' : 'hover:bg-gray-800/50'
                }`}
              >
        ...
        {/* Conversation */}
        <div className="bg-gray-900 rounded-2xl border border-gray-800 flex flex-col min-h-[70vh]">
          {!selected ? (
            ...
            <>
              <div className="px-5 py-4 border-b border-gray-800 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <h2 className="font-semibold text-white truncate">{selected.subject}</h2>
                  ...
                  <div key={m.id} className={`flex ${m.authorRole === 'admin' ? 'justify-end' : 'justify-start'}`}>
                    <div
                      className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${
                        m.authorRole === 'admin'
                          ? 'bg-amber-500 text-gray-950 rounded-tr-sm'
                          : 'bg-gray-800 text-gray-100 rounded-tl-sm'
                      }`}
                    >
                      <p className="whitespace-pre-wrap">{m.body}</p>
                      <p className={`text-[10px] mt-1 ${m.authorRole === 'admin' ? 'text-amber-900/70' : 'text-gray-500'}`}>
              ...
              <div className="p-4 border-t border-gray-800 flex items-end gap-2">
                <textarea
                  ...
                  className="flex-1 bg-gray-800 border border-gray-700 rounded-xl px-3 py-2 text-sm text-white placeholder-gray-500 outline-none focus:border-amber-500 resize-none"
                />
                <button
                  onClick={sendReply}
                  disabled={pending || !reply.trim()}
                  className="bg-amber-500 text-gray-950 font-semibold p-2.5 rounded-xl hover:bg-amber-400 transition-colors disabled:opacity-50"
                >
