...
export default function EmailSettingsClient({ initial }: { initial: EmailSettings | null }) {
  ...
  const inputCls = 'w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-white placeholder-gray-500 outline-none focus:border-amber-500';
    ...
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Mail className="w-6 h-6 text-amber-400" /> Email Settings
        </h1>
        <p className="text-sm text-gray-400 mt-1">Configure how the platform sends emails (password resets, etc.).</p>
      </div>
      ...
      <section className="bg-gray-900 rounded-2xl border border-gray-800 p-6 space-y-4">
        ...
        {s.provider === 'smtp' && (
          <div className="space-y-4 border-t border-gray-800 pt-4">
            <div className="flex items-center gap-2 text-amber-400 text-sm font-medium"><ServerCog className="w-4 h-4" /> SMTP server</div>
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="flex items-end">
                <label className="flex items-center gap-2 text-sm text-gray-300 pb-2">
                  <input type="checkbox" checked={s.secure} onChange={e => field('secure', e.target.checked)} className="w-4 h-4 accent-amber-500" />
        ...
        <button onClick={save} disabled={pending} className="bg-amber-500 text-gray-950 text-sm font-semibold px-5 py-2 rounded-lg hover:bg-amber-400 transition-colors disabled:opacity-50">
          {pending ? 'Saving…' : 'Save Settings'}
        </button>
      </section>

      <section className="bg-gray-900 rounded-2xl border border-gray-800 p-6 space-y-3">
        <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wide">Send a test email</h2>
        <div className="flex gap-2">
          ...
          <button onClick={test} disabled={pending} className="shrink-0 inline-flex items-center gap-1.5 bg-gray-800 border border-gray-700 text-white text-sm font-medium px-4 py-2 rounded-lg hover:border-amber-500 transition-colors disabled:opacity-50">
