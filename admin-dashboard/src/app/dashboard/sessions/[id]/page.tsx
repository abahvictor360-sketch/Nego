  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div className="max-w-[72%] space-y-1">
        ...
        <p className={`text-xs text-gray-400 ${isUser ? 'text-right' : 'text-left'}`}>{time}</p>
      </div>
    </div>
  );
}
    ...
    <div className="max-w-3xl">
      ...
      <div className="flex items-start justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 font-mono">{session.id}</p>
        </div>
        {session.status === 'agreed' && session.checkoutUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 text-sm font-semibold text-violet-600 hover:text-violet-800 bg-violet-50 hover:bg-violet-100 px-4 py-2 rounded-lg transition-colors whitespace-nowrap"
          >
      ...
      <div className="flex flex-wrap gap-4 text-sm text-gray-500 mb-6">
        <span>Channel: <span className="text-gray-700 capitalize font-medium">{session.channel}</span></span>
        <span>Started: <span className="text-gray-700 font-medium">{new Date(session.startedAt).toLocaleString()}</span></span>
        {session.endedAt && (
          <span>Ended: <span className="text-gray-700 font-medium">{new Date(session.endedAt).toLocaleString()}</span></span>
        )}
        <span>Messages: <span className="text-gray-700 font-medium">{visibleMessages.length}</span></span>
      </div>

      {/* Chat transcript */}
      <div className="bg-gray-50 rounded-2xl border border-gray-200 p-4 space-y-3 min-h-[200px]">
