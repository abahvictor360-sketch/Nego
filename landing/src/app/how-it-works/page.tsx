            ...
            <div className="grid sm:grid-cols-3 gap-6">
              {channels.map(({ Icon: C, title, text }) => (
                <div key={title} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm text-center">
                  <span className="inline-flex w-12 h-12 rounded-xl bg-violet-50 text-violet-600 items-center justify-center mb-4">
                    <C className="w-6 h-6" />
                  </span>
                  <h3 className="font-bold text-gray-900 mb-1.5">{title}</h3>
                  <p className="text-sm text-gray-500 leading-relaxed">{text}</p>
...
<script src="https://nego-5ykj.onrender.com/nego-widget.umd.js"></script>`}</pre>
        ...
        <section className="py-16 px-6 bg-violet-600 text-center">
          ...
          <p className="text-violet-100 mb-8">Start free — no credit card required.</p>
