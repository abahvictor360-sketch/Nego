    ...
    <>
      ...
      <main className="pt-16">
          ...
          <div className="max-w-5xl mx-auto grid sm:grid-cols-3 gap-6">
            {points.map(({ Icon, title, text }) => (
              <div key={title} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                <span className="inline-flex w-11 h-11 rounded-xl bg-violet-50 text-violet-600 items-center justify-center mb-4">
        ...
        <section className="py-16 px-6 bg-violet-600 text-center">
          ...
          <p className="text-violet-100 mb-8">Set up your own negotiating bot in minutes — free to start.</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              href="https://nego-admin.vercel.app/signup"
              className="inline-flex items-center justify-center gap-2 bg-white text-violet-600 font-semibold px-8 py-4 rounded-full hover:bg-violet-50 transition-colors shadow-lg"
            >
              Get Started Free <ArrowRight className="w-4 h-4" />
            </Link>
            <Link
              href="/how-it-works"
              className="inline-flex items-center justify-center gap-2 bg-violet-500 text-white font-semibold px-8 py-4 rounded-full border border-white/20 hover:bg-violet-400 transition-colors"
            >
              See how it works
            </Link>
          </div>
