        ...
        <div className="flex flex-col lg:flex-row gap-12 items-center">
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={tip} className="flex items-center gap-3 text-sm text-gray-600">
                  <span className="w-6 h-6 rounded-full bg-green-100 text-green-600 flex items-center justify-center shrink-0"><ArrowRight className="w-3.5 h-3.5" /></span>
                  {tip}
                </li>
              ))}
            </ul>
          ...
          <div className="lg:w-1/2 w-full">
            <div className="max-w-sm mx-auto">
              {status === 'idle' ? (
                /* Launch button */
                <div className="bg-gradient-to-br from-green-50 to-green-50 border border-green-100 rounded-2xl p-8 text-center shadow-lg shadow-green-50">
                  ...
                  <button
                    onClick={startDemo}
                    className="w-full bg-green-600 text-white font-semibold py-3 rounded-xl hover:bg-green-700 transition-colors shadow-md shadow-green-200 inline-flex items-center justify-center gap-2"
                  >
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-2xl shadow-gray-100 overflow-hidden flex flex-col" style={{ height: '480px' }}>
                    ...
                    {loading && (
                      <div className="flex justify-start">
                        <div className="bg-white border border-gray-200 rounded-2xl rounded-bl-sm px-3 py-3 flex gap-1 shadow-sm">
                          <span className="w-2 h-2 rounded-full bg-green-400 dot1" />
                          <span className="w-2 h-2 rounded-full bg-green-400 dot2" />
                          <span className="w-2 h-2 rounded-full bg-green-400 dot3" />
                        </div>
                      </div>
                    )}
                  ...
                  <div className="px-3 py-2.5 border-t border-gray-100 bg-white flex gap-2">
                    <input
                      ...
                      className="flex-1 text-sm rounded-xl border border-gray-200 px-3 py-2 outline-none focus:border-green-400 focus:ring-1 focus:ring-green-200 disabled:bg-gray-50 disabled:text-gray-400"
                    />
                    <button
                      onClick={send}
                      disabled={isDone || loading || !input.trim()}
                      className="bg-green-600 text-white rounded-xl px-3 py-2 text-sm font-semibold disabled:opacity-40 hover:bg-green-700 transition-colors"
                    >
                      Send
                    </button>
                  </div>
